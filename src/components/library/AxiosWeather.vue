...
<template>
  <div class="practice-section">
    <h2>4.1 Axios 통신 검증</h2>
    <button @click="handleFetch" :disabled="isLoading">
      {{ isLoading ? "데이터 로딩 중..." : "실시간 날씨 데이터 당겨오기" }}
    </button>
    <div v-if="Data" class="result-card">
      <p>
        📍 위치: <strong>{{ Data.name }}</strong>
      </p>
      <p>
        🌡️ 현재 기온: <strong>{{ Data.main.temp }}°C</strong> (정상 섭씨 변환
        완료)
      </p>
      <p>
        ☁️ 날씨 상태: <strong>{{ Data.weather[0].description }}</strong>
      </p>
      <p>
        💧 습도: <strong>{{ Data.main.humidity }}%</strong>
      </p>
    </div>
    <div v-else>
